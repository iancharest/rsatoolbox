...
modelRDMs_SL_sim = rsa.util.directLoad([returnHere,filesep,'sampleMask_org.mat'], 'modelRDMs_SL_sim');
anatVol = rsa.util.directLoad([returnHere,filesep,'anatomy.mat'], 'anatVol');% load the resliced structural image
...
for subjectI = 1:Nsubjects
    rs = rsa.util.directLoad([userOptions.rootPath,filesep,'Maps',filesep,'rs_subject',num2str(subjectI),'.mat'], 'rs');
    rMaps{subjectI} = rs;
